...
class LoadData(object):
    ...
    def _populate_calculated_project_fields(self):
        ...
        # Slightly slower, but more readable code:

        # for proj in proj_ids:
        #     lines_all = []
        #     for row in results:
        #         if row.nlihc_id == proj:
        #             lines_all.append(row.lines)
        #     lines_all = ':'.join(lines_all)
        #     lines_all = lines_all.split(':')
        #     routes = len(set(lines_all))
        #     bus_routes_nearby.append({'proj':proj,'routes':routes})

        upd = p.update()\
            .where(p.c.nlihc_id == bindparam('proj'))\
            .values(bus_routes_nearby = bindparam('routes'))

        r1 = conn.execute(upd,bus_routes_nearby)
